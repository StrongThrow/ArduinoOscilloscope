...
class _LEDScreenState extends State<LEDScreen> {
  ...
  void didChangeDependencies() {
    ...
    preCacheImages();
    context.read<BluetoothCubit>().sendMessage('e*');
  }

  Future<void> preCacheImages() async {
    for (int i = 0; i < 35; i++) {
      String imagePath = 'assets/led/${i.toString().padLeft(6, '0')}.png';
              ...
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => NoteScreen(assetsPath: 'assets/md/led.md',)));
                  },
                  child: Container(
                    height: height / 2,
                  ...
                  Expanded(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text('LED 애니메이션', style: TextStyle(fontSize: 20, fontFamily: 'SpoqaHanSansNeo', fontWeight: FontWeight.bold),),
                          Divider(),
                          SizedBox(height: 20,),
                              ...
                              child: BlocListener<BluetoothCubit, BluetoothCustomState>(
                                listener: (context, state) {
                                  if (state.receivedMessage.isNotEmpty) {
                                    ...
                                    if (receivedValue != null && receivedValue >= 0.0 && receivedValue <= 5.0) {
                                      setState(() {
                                        volte = receivedValue;
                                      });
                                      // 메시지를 애니메이션의 totalProgress에 맞게 변환
                                      final progress = (volte / 5.0) * _animatorState!.totalProgress;
                                      _animatorState?.skip(progress); // 애니메이션 진행 변경
                                    }
                                  }
                                },
                                child: Column(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: [
                                    Text('⚡현재 전압: ${volte}V', style: TextStyle(fontFamily: 'SpoqaHanSansNeo', fontWeight: FontWeight.bold),),
                                    SizedBox(height: 10,),
                                    progressBar(volte),
                                  ],
                                ),
